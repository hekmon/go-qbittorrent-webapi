...
type ApplicationPreferences struct {
	...
	ScanDirs                           map[string]any          `json:"scan_dirs,omitempty"`                              // Directories to watch for torrent files, value: where torrents loaded from this directory should be downloaded to. Slashes are used as path separators. Keys are paths. Possibles values 0: Download to the monitored folder, 1: Download to the default save path, <string>: a destination path
	...
	BittorrentProtocol                 *BittorrentProtocol     `json:"bittorrent_protocol,omitempty"`                    // Bittorrent Protocol to use
	...
	SchedulerDays                      *SchedulerDays          `json:"scheduler_days,omitempty"`                         // Scheduler days. See possible values here below
	...
	Encryption                         *EncryptionMode         `json:"encryption,omitempty"`                             // Transmission encryption usage
	...
	ProxyType                          *ProxyType              `json:"proxy_type,omitempty"`                             // Proxy usage
	...
	UploadChokingAlgorithm             *UploadChokingAlgorithm `json:"upload_choking_algorithm,omitempty"`               // Upload choking algorithm used (see list of possible values below)
	UploadSlotsBehavior                *UploadSlotsBehavior    `json:"upload_slots_behavior,omitempty"`                  // Upload slots behavior used (see list of possible values below)
	UPnPLeaseDuration                  *int                    `json:"upnp_lease_duration,omitempty"`                    // UPnP lease duration (0: Permanent lease)
	UTPTCPMixedMode                    *UTPTCPMixedMode        `json:"utp_tcp_mixed_mode,omitempty"`                     // μTP-TCP mixed mode algorithm (see list of possible values below)
}
...
func (pt ProxyType) String() string {
	...
	case ProxyHTTPNoAuth:
		return "HTTP proxy without authentication"
	case ProxySOCKS5NoAuth:
		return "SOCKS5 proxy without authentication"
	case ProxyHTTPAuth:
		return "HTTP proxy with authentication"
	case ProxySOCKS5Auth:
		return "SOCKS5 proxy with authentication"
	case ProxySOCKS4NoAuth:
		return "SOCKS4 proxy without authentication"
	default:
		return "Unknown"
	}
}
...
func (m MaxRatioAct) String() string {
	switch m {
	case MaxRatioActPauseTorrent:
		return "Pause torrent"
	case MaxRatioActRemoveTorrent:
...
func (b BittorrentProtocol) String() string {
	...
	case BittorrentProtocolTCPAndUTP:
		return "TCP and μTP"
	case BittorrentProtocolTCP:
		return "TCP"
	case BittorrentProtocolUTP:
		return "μTP"
	default:
		return "Unknown"
	}
...
func (u UploadChokingAlgorithm) String() string {
	switch u {
	case UploadChokingAlgorithmRoundRobin:
		return "Round-robin"
	case UploadChokingAlgorithmFastestUpload:
		return "Fastest upload"
	case UploadChokingAlgorithmAntiLeech:
		return "Anti-leech"
	default:
		return "Unknown"
	}
...
func (u UploadSlotsBehavior) String() string {
	switch u {
	case UploadSlotsBehaviorFixedSlots:
		return "Fixed slots"
	case UploadSlotsBehaviorUploadRateBased:
